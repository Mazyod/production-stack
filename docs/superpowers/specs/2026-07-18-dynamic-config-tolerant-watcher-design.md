...
| `--enable-structured-output-repair` | `app.state.structured_output_repair_enabled` |
| `--structured-output-repair-max-bytes` | `app.state.structured_output_repair_max_bytes` |
| `--structured-output-repair-max-seconds` | `app.state.structured_output_repair_max_seconds` |
| `--structured-output-repair-capture-dir` | capture sink |
| `--structured-output-repair-capture-sample-rate` | capture sink |
| `--structured-output-repair-capture-max-bytes` | capture sink |
| `--structured-output-repair-capture-retention-days` | capture sink |
...
- **Recognized startup-only flags** — any argparse destination that is not a reconfigurable field (the 9 fork flags, plus upstream flags like `engine_stats_interval`). Honored already at startup; ignored by the watcher and logged at `debug`.
...
- **vs. enumerating the 9 flags as `DynamicRouterConfig` fields** (the literal `timeout_keep_alive` precedent): tolerance covers *all* flags with zero per-flag maintenance, now and for any future flag. It is also safer on the spurious-reconfigure axis (below).
...
### The 9 fork flags are inert to the watcher — the key property

The watcher reconfigures on `config != self.current_config`, where both operands are `DynamicRouterConfig` instances. The 9 fork flags never enter the dataclass on **either** side — neither `from_args(args)` (reads only dataclass fields) nor `from_yaml(path)` (classifies them as startup-only and skips them). Therefore editing a fork key at runtime does **not** fire a reconfigure (both operands filter it identically), and adding fork keys is neutral to the first-tick comparison.

`timeout_keep_alive` is the one exception: it remains a reconfigurable dataclass field (per the earlier precedent), so editing *it* at runtime still fires a disruptive no-op `reconfigure_all`. That is a pre-existing minor and is out of scope; the inertness claim is scoped to the 9 fork flags.
...
1. **Startup-only.** The 9 fork flags are read once at boot. Editing them in the file while the router runs has no effect until restart (they are inert to the watcher, so the edit is a no-op — not even a reconfigure).
...
1. `from_yaml` accepts all 9 fork flags **plus** a sample upstream key (`engine_stats_interval`) in one file without crashing, and still parses the reconfigurable fields (`service_discovery`, `routing_logic`); `from_json` equivalent.
2. **Fork flags are inert:** two YAML files identical except that one carries all 9 fork flags produce **equal** `DynamicRouterConfig` objects via `from_yaml`.
